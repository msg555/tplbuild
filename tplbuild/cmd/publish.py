...
from tplbuild.images import MultiPlatformImage
...
class PublishUtility(CliUtility):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            "override the default push tags for stage_name or "
    ...
    async def main(self, args, tplbld: TplBuild) -> int:
        ...
        for platform in platforms:
            ...
            for stage_name, stage_data in stage_mapping.items():
                ...
                if multi_stage.push_tags != stage_data.push_tags:
                    raise TplBuildException(
                        "Push tags must match for all platforms for stage {repr(stage_name)}"
                    )
        ...
        for image_arg in args.image:
            ...
            if len(image_parts) > 1:
                multi_stage_mapping[image_parts[0]].push_tags = (
                    image_parts[1] or image_parts[0],
                )

        # Only explicitly build stages that have push_tags associated with them.
